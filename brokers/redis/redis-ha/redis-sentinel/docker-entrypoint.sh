#!/bin/bash
# sentinel节点的配置方式
# sentinel节点可以配置多个master,目前只配置一个
# REDIS_SENTINEL_MASTER_NAME   master名称
# REDIS_SENTINEL_MASTER_HOST   host
# REDIS_SENTINEL_MASTER_PORT   port
# REDIS_SENTINEL_MASTER_QUORUM   quorum 默认是2
# REDIS_SENTINEL_DOWN_AFTER_MILLISECONDS  失效时间, 单位是毫秒，默认为30秒
# REDIS_SENTINEL_FAILOVER_TIMEOUT  failover超时时间
# REDIS_SENTINEL_PARALLEL_SYNCS 主备切换时最多可以有多少个slave同时对新的master同步
# REDIS_SENTINEL_PASSWORD 主备切换时最多可以有多少个slave同时对新的master同步
    ...
    if [ ! -f "$CONFIG" ]; then
        ...
        if [ $USE_REDIS_SENTINEL_CONF_FILE == "true" ];then
            ...
            master_name=${master_name:-'mymaster'}
             ...
             # master host
            key='REDIS_SENTINEL_MASTER_HOST'
            file_env key
            value=${!key}
            if [ -n "$value" ];then
                master_host=$value
            ...
            # master port
            key='REDIS_SENTINEL_MASTER_PORT'
            file_env key
            value=${!key}
            if [ -n "$value" ];then
            ...
            # master quorum
            key='REDIS_SENTINEL_MASTER_QUORUM'
            file_env key
            value=${!key}
            if [ -n "$value" ];then
                if grep '^[[:digit:]]*$' <<< "$value";then
           ...
           echo  "sentinel monitor $master_name $master_host $master_port $master_quorum" >> "$CONFIG"
            ...
            # 同步数量
            key='REDIS_SENTINEL_PARALLEL_SYNCS'
            file_env key
            value=${!key}
            if [ -n "$value" ];then
                # 大于等于0的数字
